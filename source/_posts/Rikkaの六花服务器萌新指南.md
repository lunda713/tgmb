...
萌新的发展**首推匠魂**，其次是植物魔法。匠魂可以**提升冶炼原矿的产量（双倍）**，同时还能制作很多有趣的工具。植物魔法则已经隐隐成为星系的前置mod了，具体后面会解释。
...
*备注：该部分附属于IC2mod，提前说一句————————设备上可以优先做一个**MFE储电箱**，然后制作纳米套装（MFE储电箱的提供的512Eu/t的电压才能给纳米套装充电，更高电压自然更好），这里的纳米头盔需要用到新手礼包里的夜视镜。*
...
（服务器ban了魔力脉冲枪，而精卫之羽、万藏酒的伤害吸收buff表现形式已从原来的免疫伤害形式改为抵消伤害，而瞬间治愈的药水透镜同样会给盖亚III加血，所以那些还活在老版本的“高手玩家”们就不要来炫耀什么盖亚III很好打了，解释起来挺累的）
...
这个技巧其实并不难，原理在于过剩魔力供给和空手右键返回上一配方。
如图摆放
...
| :--: | :--: |
|   1阶-3阶   |    1阶（铝装甲层）  |
|    4阶-5阶  |     2阶（钛装甲层，钛铁本该是在小行星带的，但本服小行星带没有钛铁矿，获取钛只能是铝矿石打粉洗矿热能离心，或者在金星上的坠毁探测器中捡到的致密钛板打粉） |   
|   6阶-7阶   |   3阶（钯装甲层）   |  
|     8阶-9阶  |   4阶（锌装甲层）     |

（上太空需要太空服和热防护服，毕竟没有太空服无法在外太空生存。热防护服JEI按住SHIFT可以查看对应星球，所以不做赘述。太空服并不是完全防护的，尤其是小行星带会有强辐射，必须带好回血药剂或魔法道具）
...
<div class="note default no-icon"><p>
小行星的一些区域存在高辐射，会以远比正常情况高的速率扣血（正常辐射扣血量叠加虚空扣血量）。如果是降落点就是这样，那么请更换发射点。如果是在寻找钻石小行星的途中突然遇到高辐射区域，那么只能一刻不停的喝万藏无限之酒，然后拔高高度，尽快脱离当前区域。
</p></div>
...
黑洞护符的取出一直困扰着我，毕竟就算是1组1组的取，在后期挖矿大户的眼中还是太慢了，所以要自动化。动机明显，目的明确。
...
# 最初也是最后的礼物：AE+IC2自动化车间

<div class="note no-icon info"><p>
...
给洗矿机(两个都要配置输出总线）、热能离心机(两个都要配置输出总线）、电炉(选一个配置输出总线）、打粉机(选两个个配置输出总线）、压缩机（选一个配置输出总线）、回收机（给唯一的一个配置输出总线）配置上输出总线完成流水线。各个机器的升级插件均在**3.5.4廉价的超频升级组件**和**4.4自动化矿物处理流水线**中指明。回收机和其中一个洗矿机的频道需要向ME控制器再接一个。
...
7. 分子装配室结构体，需要10根ME致密线缆和64个ME接口和64个分子装配室。ME接口和分子装配室的组合占一个频道，所以最多可以搭32个组合，交错堆叠放置能把结构体的理由律提升到最大。
...
<div class="note no-icon danger"><p>
